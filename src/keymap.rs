...
macro_rules! normalized_char {
    ($ch:expr) => {
        if $ch.is_ascii_uppercase() {
        	KeyEvent{code: KeyCode::Char($ch), modifiers: KeyModifiers::SHIFT}
    	} else {
        	KeyEvent{code: KeyCode::Char($ch), modifiers: KeyModifiers::NONE}
    	}
    }
}
...
macro_rules! k {
	...
	($map:ident, (key $key:path => $act:expr)) => {
		$map.insert(KeyEvent{code: $key, modifiers: KeyModifiers::NONE}, $act);
	};
}
